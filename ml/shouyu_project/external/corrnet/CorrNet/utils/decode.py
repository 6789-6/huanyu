...
class Decode(object):
    def __init__(self, gloss_dict, num_classes, search_mode, blank_id=0):
        ...
        if self.search_mode != "max" and ctcdecode is not None:
    ...
    def MaxDecode(self, nn_output, vid_lgt):
        ...
        for batch_idx in range(batchsize):
            cur_lgt = int(vid_lgt[batch_idx].item() if hasattr(vid_lgt[batch_idx], "item") else vid_lgt[batch_idx])
            group_result = [x[0] for x in groupby(index_list[batch_idx][:cur_lgt])]
